...
import { FilenameColumnID, type RawTagEditCell, type RawTagEditRow } from './tag-editor.types';
import { extractFileExtension, replaceFileSystemChars, splitFilename } from './utils';

export interface MediaTagRawFramesAccess extends Jam.MediaTagRawFrames {
	[key: string]: Array<Jam.MediaTagRawFrame> | undefined;
}

export function rebuildTag(edit: RawTagEditRow): Jam.MediaTagRaw {
	const frames: MediaTagRawFramesAccess = {};
...
export function getPartOfSetID(edit: RawTagEditRow): string {
	const partOfSet: RawTagEditCell<{ text: string }> | undefined = edit.cells.find(f => f.column.def.id === 'TPOS');
