...
import type { MediaTagRawFramesAccess } from './tag-editor.utils';

export class ID3V2RawBuilder {
	private frameValues: MediaTagRawFramesAccess = {};

	build(): MediaTagRawFramesAccess {
		return this.frameValues;
	}

	text(key: string, text: string | undefined): this {
		if (text) {
			const frame: Jam.MediaTagRawFrameText = { id: key, value: { text } };
	...
	idText(key: string, id: string, value: string | undefined): this {
		if (value) {
			const frame: Jam.MediaTagRawFrameIdText = { id: key, value: { id, text: value } };
			const list = ((this.frameValues[key] ?? []) as Array<Jam.MediaTagRawFrameIdText>)
	...
	keyTextList(key: string, group: string, value?: string): this {
		if (value) {
			const frames = (this.frameValues[key] ?? []) as Array<Jam.MediaTagRawFrameTextList>;
			const frame: Jam.MediaTagRawFrameTextList = (frames.length > 0) ? frames[0] : { id: key, value: { list: [] } };
	...
	bool(key: string, bool?: boolean): this {
		if (bool !== undefined) {
			const frame: Jam.MediaTagRawFrameBool = { id: key, value: { bool } };
			this.frameValues[key] = [frame];
		}
		return this;
	}
	...
	idLangText(key: string, value: string | undefined, lang: string | undefined, id: string | undefined): this {
		if (value) {
			const cid = id ?? '';
			const list = ((this.frameValues[key] ?? []) as Array<Jam.MediaTagRawFrameLangDescText>)
				.filter(f => f.value.id !== cid);
			const frame: Jam.MediaTagRawFrameLangDescText = { id: key, value: { id: cid, language: lang ?? '', text: value } };
			this.frameValues[key] = [...list, frame];
		}
		return this;
	}

	addPicture(key: string, pictureType: number, description: string, mimeType?: string, binary?: string): this {
		const frame: Jam.MediaTagRawFramePic = {
			id: key, value: {
				description,
				pictureType,
				bin: binary,
	...
	addChapter(key: string, chapterID: string, start: number, end: number, offset: number, offsetEnd: number, subframes?: Array<Jam.MediaTagRawFrame>): this {
		const frame: Jam.MediaTagRawFrameChapter = {
			id: key,
			value: {
				id: chapterID,
				start,
...
export class ID3V24TagBuilder {
	...
	buildFrames(): MediaTagRawFramesAccess {
	...
	addPicture(pictureType: number, description: string, mimeType?: string, binary?: string): this {
		this.rawBuilder.addPicture('APIC', pictureType, description, mimeType, binary);
		return this;
	}

	chapter(id: string, start: number, end: number, offset: number, offsetEnd: number, subframes?: Array<Jam.MediaTagRawFrame>): this {
