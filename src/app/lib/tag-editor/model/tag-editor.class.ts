import { MUSICBRAINZ_VARIOUS_ARTISTS_ID, MUSICBRAINZ_VARIOUS_ARTISTS_NAME } from '@utils/jam-lists';
...
export class TagEditor {
	...
	updateEditTextCell(edit: RawTagEditRow, column: RawTagEditColumn, value?: string): void {
		const index = this.columns.indexOf(column);
		const cell: RawTagEditCell<{ text: string } | { id: string; language: string; text: string }> | undefined = edit.cells[index];
		const text = value ?? '';
	...
	setAlbumArtistFrames(column: RawTagEditColumn): void {
		...
		if (artistColIndex !== -1) {
			for (const edit of this.edits) {
				const cell = edit.cells[artistColIndex] as RawTagEditCell<{ text: string }>;
	...
	getCellText(cell: RawTagEditCell<any>): string | undefined {
		return cell.frames.length > 0 ? (cell as RawTagEditCell<{ text: string }>).frames[0].value.text : undefined;
	}
	...
	insertFromColumn(sourceColIndex: number, pos: number, column: RawTagEditColumn): void {
		...
		if (sourceColIndex >= 0) {
			for (const edit of this.edits) {
				const sourceText = this.getCellText(edit.cells[sourceColIndex] as RawTagEditCell<{ text: string }>);
				const destText = this.getCellText(edit.cells[index] as RawTagEditCell<{ text: string }>) ?? '';
				if (sourceText) {
	...
	copyColumn(sourceColIndex: number, column: RawTagEditColumn): void {
		if (sourceColIndex >= 0) {
			for (const edit of this.edits) {
				const cell = edit.cells[sourceColIndex] as RawTagEditCell<{ text: string }>;
				const text = cell.frames.length > 0 ? cell.frames[0].value.text : undefined;
				if (text) {
					this.updateEditTextCell(edit, column, text);
				}
	...
	upgradeDateFramesTov24Date(frames: Array<Jam.MediaTagRawFrame>): {
		newFrame: Jam.MediaTagRawFrame;
		dateFrames: Array<Jam.MediaTagRawFrame>;
	} | undefined {
		...
		const dateFrames: Array<Jam.MediaTagRawFrame> = [];
		const result: Array<string> = [];
		if (year?.value && Object.hasOwn(year.value, 'text')) {
			dateFrames.push(year);
			result.push((year as Jam.MediaTagRawFrameText).value.text);
		}
		if (date?.value && Object.hasOwn(date.value, 'text')) {
			dateFrames.push(date);
			result.push((date as Jam.MediaTagRawFrameText).value.text);
		}
		if (time?.value && Object.hasOwn(time.value, 'text')) {
			dateFrames.push(time);
			result.push((time as Jam.MediaTagRawFrameText).value.text);
		}
		return {
			newFrame: {
				id: 'TDRC',
	...
	private getAutoCompleteList(column: RawTagEditColumn, cell?: RawTagEditCell<any>): Array<string> {
		...
		for (const edit of this.edits) {
			const c = edit.cells[index] as RawTagEditCell<{ text: string }>;
	...
	private frame2Column(frame: RawTagEditFrame<{ id: string }>, sort: number): RawTagEditColumn {
		const subid = frame.value.id ? ` - ${frame.value.id as string}` : '';
		const col: RawTagEditColumn = {
			def: {
				id: frame.id,
				name: `${frame.id}${subid}`,
		...
		col.getAutoCompleteList = (cell?: RawTagEditCell<any>): Array<string> => this.getAutoCompleteList(col, cell);
		return col;
	}
	...
	private findExistingColumn(frame: RawTagEditFrame<any>): RawTagEditColumn | undefined {
		return this.columns.find(c => TagEditor.matchColumn(frame, c.def));
	}

	private createColumnFromFrame(frame: RawTagEditFrame<{ id: string }>): void {
		const framedef = getFrameDef(frame.id);
		const sort = DefaultFrameColumns.findIndex(c => TagEditor.matchColumn(frame, c));
		if (framedef) {
			this.columns.push(this.frameDef2Column(frame.id, frame.value.id, framedef, sort));
		} else {
	...
	private buildColumns(tracks?: Array<Jam.Track>): void {
		...
		const getTrackFrames = (track: Jam.Track): Array<RawTagEditFrame<{ id?: string }>> => {
			...
			return frames as Array<RawTagEditFrame<{ id?: string }>>;
		};
		...
		const processTrackFrames = (track: Jam.Track): void => {
			...
			for (const frame of frames) {
				...
				if (!existingColumn) {
					this.createColumnFromFrame(frame as RawTagEditFrame<{ id: string }>);
				}
			}
		};
	...
	private buildEdits(tracks: Array<Jam.Track>): void {
		...
		const fillColumns = (
			...
			frames: Array<RawTagEditFrame<any>>,
			parent: RawTagEditRow,
			oldRow: RawTagEditRow | undefined): Array<RawTagEditCell<any>> =>
