...
export class JamBaseService {
	...
	async raw(view: string, params: any): Promise<{ buffer: ArrayBuffer; mimeType: string }> {
	...
	async binary(path: string, params?: any): Promise<{ buffer: ArrayBuffer; mimeType: string }> {
		if (!this.authService.isLoggedIn()) {
			return Promise.reject(Error('Not logged in'));
		}
